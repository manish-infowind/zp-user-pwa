...
export function Logo({ compact = false, className }: LogoProps) {
  return (
    <span className={cn('inline-flex items-center gap-2', compact ? 'gap-1.5' : 'gap-2', className)}>
      <span
        className={cn(
      ...
      >
        <svg className={compact ? 'size-3.5' : 'size-4.5'} viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <span className="min-w-0">
        <span className={cn('block font-sans font-extrabold tracking-[-0.03em] text-[#3b3b3b]', compact ? 'text-[1.05rem]' : 'text-[1.2rem]')}>
          ZappieCare
        </span>
      </span>
    </span>
