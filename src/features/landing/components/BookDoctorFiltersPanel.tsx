...
type Props = {
  ...
  selectedDoctorType: (typeof landingStripDoctorTypes)[number]
  onDoctorTypeChange: (value: (typeof landingStripDoctorTypes)[number]) => void
  feeMin: number
  feeMax: number
...
/** Fake histogram bars for consultation fee (visual parity with Figma). */
const feeHistogramHeights = [32, 48, 55, 42, 38, 50, 44, 36, 58, 45, 40, 52, 47, 35, 42, 38, 45, 50, 44, 40]

export function BookDoctorFiltersPanel({
  selectedSpecialty,
  onSpecialtyChange,
  selectedDoctorType,
  onDoctorTypeChange,
  ...
  return (
    <div className={cn('rounded-2xl border border-[#e8eaee] bg-white p-4 shadow-[0_4px_24px_rgba(31,31,31,0.06)]', className)}>
      <div className="relative mb-4 overflow-hidden rounded-xl bg-[#e8edf2]">
        <div
          className="aspect-[5/3] w-full bg-[linear-gradient(135deg,#dbeafe_0%,#e0e7ef_50%,#d1d9e6_100%)]"
          role="img"
