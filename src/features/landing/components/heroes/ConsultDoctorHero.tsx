import { consultDoctorHeroCopy } from '@/features/landing/data/landingData'

import { FloatingHeroBadge } from './FloatingHeroBadge'

type ConsultDoctorHeroProps = {
  frameSrc: string
  characterSrc: string
  onBookNow: () => void
}

export function ConsultDoctorHero({ frameSrc, characterSrc, onBookNow }: ConsultDoctorHeroProps) {
  return (
        ...
        <div className="relative z-10 grid h-full min-h-[inherit] grid-cols-1 items-end gap-8 px-5 pb-7 pt-9 md:grid-cols-[minmax(240px,0.46fr)_minmax(0,1fr)] md:gap-6 md:px-8 md:pb-9 md:pt-10 lg:grid-cols-[minmax(260px,0.45fr)_minmax(0,1fr)]">
          <div className="relative flex min-h-[200px] w-full items-end justify-center md:min-h-0 md:justify-start md:pl-0">
            ...
            <img
              ...
              decoding="async"
              fetchPriority="high"
              className="relative z-30 !max-w-[min(100%,420px)] h-auto w-auto max-h-[min(340px,48vh)] object-contain object-bottom drop-shadow-[0_12px_40px_rgba(0,0,0,0.12)] md:max-h-[min(100%,360px)]"
            />
          </div>
          <div className="flex w-full flex-col items-end gap-[30px] pb-1 text-right md:max-w-[740px] md:justify-self-end md:pr-2 md:pt-2">
            <div className="flex w-full flex-col items-end gap-[clamp(1.25rem,3vw,2.2rem)]">
              <h2 className="w-full max-w-[740px] text-right font-sans text-[clamp(1.35rem,4.2vw,3rem)] font-black uppercase leading-[1.42] text-white lg:text-[48px]">
                {consultDoctorHeroCopy.headline}
              </h2>
              <p className="w-full max-w-[600px] text-right font-sans text-[clamp(0.95rem,2.2vw,1.25rem)] font-bold capitalize leading-[1.42] text-white lg:text-[22px]">
