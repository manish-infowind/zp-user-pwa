  ...
  return (
    <div className="flex items-center gap-2" aria-hidden>
      {Array.from({ length: 4 }).map((_, index) => (
        <svg key={`filled-${index}`} className="size-[1.3rem]" viewBox="0 0 26 26" fill="none">
          <path
      ...
      <svg className="size-[1.3rem]" viewBox="0 0 26 26" fill="none">
        <path
    ...
    variant === 'doctor'
      ...
      : variant === 'nurse'
        ...
        : variant === 'lab'
          ? {
              scriptColor: '#B68000',
              highlightedCardClassName: 'bg-[#FFF3D8]',
              highlightedAccentColor: '#B68000',
            }
        : null
  ...
  return (
    <section className="overflow-hidden">
      <div className="app-container">
            ...
            <div className="flex flex-wrap justify-center gap-4 px-1">
              {patientTestimonials.map((testimonial, index) => (
                <article
                  ...
                  className={cn(
                    'w-[17rem] shrink-0 rounded-[18px] px-4',
                    themedVariant && index === 1 ? themedVariant.highlightedCardClassName : testimonial.bgClassName,
                    testimonial.heightClassName,
                    index === 1 ? 'pt-4' : 'mt-[3.25rem] pt-4',
                  )}
                >
                  <div className="flex w-[14.75rem] flex-col items-start gap-3">
                    <TestimonialRating
                      color={themedVariant && index === 1 ? themedVariant.highlightedAccentColor : testimonial.starColor}
                    />
                    <p className="w-full text-[0.875rem] font-medium leading-[1.6] text-[#8D8D8D]">
                      {testimonial.quote}
                    </p>
                    <div className="flex flex-col items-start gap-1">
                      <p className="text-[1rem] font-semibold leading-[1.1] text-[#1F1F1F]">{testimonial.name}</p>
                      <p
                        className="text-[0.9rem] font-medium leading-[1.1]"
        ...
        <div className="pb-7 pt-8 lg:hidden">
          <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide">
            {patientTestimonials.map((testimonial, index) => (
              <article
                key={`${testimonial.name}-${index}-mobile`}
                className={cn(
                  'w-[16.5rem] shrink-0 rounded-[18px] p-5',
                  themedVariant && index === 1 ? themedVariant.highlightedCardClassName : testimonial.bgClassName,
                  index === 1 ? 'min-h-[22rem]' : 'min-h-[19rem]',
                )}
              >
                <div className="flex flex-col items-start gap-4">
                  <TestimonialRating
                    color={themedVariant && index === 1 ? themedVariant.highlightedAccentColor : testimonial.starColor}
                  />
                  <p className="text-[0.95rem] font-medium leading-[1.65] text-[#8D8D8D]">{testimonial.quote}</p>
                  <div className="flex flex-col items-start gap-1">
                    <p className="text-[1rem] font-semibold leading-[1.1] text-[#1F1F1F]">{testimonial.name}</p>
                    <p
                      className="text-[0.9rem] font-medium leading-[1.1]"
        ...
        <div className="bg-[#FEFAE7] py-6 text-center md:py-7 lg:h-[152px] lg:py-7">
          <p
            className={cn(
              'text-[clamp(1.15rem,2.2vw,1.75rem)] font-normal leading-[1.25]',
          ...
          <h2 className="mx-auto max-w-[52rem] px-[var(--page-gutter)] font-display text-[clamp(1.45rem,2.8vw,2.25rem)] font-bold uppercase leading-[1.1] text-[#1F1F1F]">
